...
#Output to csv file
OutputFileName="ITBLO-SentenceList.csv"
...
LearningOutcomesStrainer =SoupStrainer("table", class_="borders")
#DB Query Remember to select the correct Institution and remove the Limit 10 
GetLearningOutcomesQuery =("SELECT * FROM `text-assignment`.`allcourses_raw` WHERE `Institution` = 'ITB' and MATCH (OriginalTextFromDocument) AGAINST ('Learning Outcome' )")
...
for row in cursor: 
    ...
    OldKey = row['OldKey']
    WebTables = BeautifulSoup(TextForSoup,"html.parser",parse_only=LearningOutcomesStrainer)
    TableHeaders = WebTables.findAll("th")
    for head in TableHeaders:
        if "Learning Outcome" in head.get_text():
            #Get the parent table of table header 
            LearningOutcomesTable = head.findParent("table")
            #print(LearningOutcomesTable.prettify(formatter="html"))
            #Now I have a table get me the table rows
            LearningRows = LearningOutcomesTable.findAll("tr")
            for row in LearningRows:
                ...
                for sentence in SentenceList:
                    ...
                    #Remove unprintable characters from WordList
